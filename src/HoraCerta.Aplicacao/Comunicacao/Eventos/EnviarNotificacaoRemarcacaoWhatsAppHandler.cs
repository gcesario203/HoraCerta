...
public class EnviarNotificacaoRemarcacaoWhatsAppHandler : IDomainEventHandler<AgendamentoRemarcadoEvent>
{
    ...
    public EnviarNotificacaoRemarcacaoWhatsAppHandler(
    ...
    {
        _outboxRepositorio.CancelarPorAgendamento(evento.AgendamentoAnteriorId);
        ...
        var payload = JsonSerializer.Serialize(new OutboxPayloadDto(evento.NovoAgendamentoId));

        _enfileirador.Enfileirar(
            TipoMensagemOutbox.NotificacaoRemarcacao,
            evento.TelefoneCliente,
            evento.ProprietarioId,
            MensagensWhatsAppTemplates.Remarcacao(evento.NovoSlotInicio),
            $"Remarcacao:{evento.NovoAgendamentoId}",
            payload);
    }
}
